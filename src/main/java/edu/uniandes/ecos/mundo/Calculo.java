...
public class Calculo {
    ...
    /**
     * Método que retorna el valor resultante al aplicar la distribución p en la
    ...
    public double cargarIntegracionNumerica(Integracion integracion) {
        ...
        do {
            ...
            for (int i = 0; i <= numeroDegmentos; i++) {                
                double ancho = integracion.getValX() / numeroDegmentos;
                int multiplicador;
                double xi = ancho * i;
            ...
            if (phistorico == 0 || (Math.abs((phistorico - p)) > error)) {
                numeroDegmentos = numeroDegmentos + numeroDegmentos;
                iteracionAdicional = true;                
            } else {
                iteracionAdicional = false;
                return phistorico;
            }
